...
Â~(This should be the default choice.)
...
\end{tabular}
 Description \\
{\footnotesize
Select the catalog table to be queried (see test parameter). Â~
...
screen. Â~(Note that the test dataset is likely to be of limited extent
...
search screen. Â~Note in the test report whether a subset of the
...
Â~(It is not a requirement of DP0.1 for a coverage image for DESC DC2 to
...
the expected set of columns is included in the query result. Â~Record
the number of rows returned by the query.\\[2\baselineskip]Record the
...
tabular query result as an attachment to the test record. Â~Use the
...
\end{tabular}
 Description \\
{\footnotesize
Click on the `i'-in-a-circle button in the table viewer. Â~Use the
